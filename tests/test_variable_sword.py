...
class VarSwordTests(unittest.TestCase):
    def test_adds_bn3_element_sonic_command_for_both_sides(self) -> None:
        source = (ROOT / "src/chips/variable_sword.c").read_text()
    ...
    def test_element_waves_cycle_fire_elec_wood_aqua(self) -> None:
        source = (ROOT / "src/chips/variable_sword.c").read_text()
        dispatch = source[source.index("NAKED void variable_sword_element_wave_dispatch"):]
    ...
    def test_only_element_sonic_repeats_four_times(self) -> None:
        source = (ROOT / "src/chips/variable_sword.c").read_text()
        init = source[source.index("NAKED void variable_sword_sonic_boom_init"):]
    ...
    def test_keeps_all_variable_sword_patch_assembly_with_its_c_package(self) -> None:
        source = (ROOT / "src/chips/variable_sword.c").read_text()
        hooks = (ROOT / "src/hooks.asm").read_text()

        self.assertNotIn("VarSword", hooks)
        self.assertNotIn("variable_sword_", hooks)
        self.assertIn(
    ...
    def test_only_final_element_wave_requests_flashing_invulnerability(self) -> None:
        source = (ROOT / "src/chips/variable_sword.c").read_text()
        finalize = source[source.index("NAKED void variable_sword_sonic_shell_finalize"):]
        modifier = source[source.index("NAKED void variable_sword_sonic_hit_modifier"):]
