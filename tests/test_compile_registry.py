...
class PackageCompilerTests(unittest.TestCase):
    ...
    def test_django_restores_bn5_base_attack_in_missing_slots(self) -> None:
        ...
        crossover_patch = (ROOT / "src/chips/django.asm").read_text()
        common_asm = (ROOT / "src/common.asm").read_text()
        self.assertIn('.include "src/chips/django.asm"', common_asm)
        self.assertIn(".org 0x080BA47C", crossover_patch)
        self.assertIn(".org 0x080BA481", crossover_patch)
        self.assertIn(".org 0x080B8C0C", crossover_patch)
        self.assertIn(".org 0x080B8C11", crossover_patch)
        self.assertEqual(crossover_patch.count(".db 0x0F"), 4)
